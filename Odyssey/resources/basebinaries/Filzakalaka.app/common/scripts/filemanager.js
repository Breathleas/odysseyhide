...
var setUploader = function(path) {
	...
	$('#uploader').attr('action', path);
	if (path != null)
 	{
 		window.history.replaceState(null, 'Filza File Manager', path);
 	}
 	console.log("setUploader: " + path);
...
var bindToolbar = function(data) {
	...
	if (!has_capability(data, 'download')) {
		...
		$('#fileinfo').find('button#download').click(function(){
			window.location = encodeURI(data['Path']) + '?mode=download';
		}).show();
	}
};
...
var addFolder = function(parent, name, preview) {
	var newNode = '<li class="directory collapsed" style="background-image: url(\''+ preview + '\');background-size: 16px 16px; background-repeat: no-repeat;"><a data-path="' + parent + name + '/" href="#">' + name + '</a><ul class="jqueryFileTree" style="display: block;"></ul></li>';
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + path + '&time=' + d.getMilliseconds(), function(data){
		...
		switch(action){
			...
			case 'download': // todo implement javascript method to test if exstension is correct
				window.location = encodeURI(data['Path']) + '?mode=download' + '&time=' + d.getMilliseconds();
				break;
...
$(function(){
    ...
    expandedFolder = decodeURI(window.location.pathname);
    fullexpandedFolder = decodeURI(window.location.pathname);
...
//				// seems to be necessary when dealing w/ files located on s3 (need to look into a cleaner solution going forward)
	...
	{
		getDetailView(expandedFolder);
		$('#uploader').attr('action', expandedFolder);
	}
	else
	{
		getDetailView(fileRoot + expandedFolder);
		$('#uploader').attr('action', fileRoot + expandedFolder);
	}
...
//     		console.log('~> state: ' + state[0] + ' type: ' + typeof state[0]);
//     		shouldPushState = false;
//         	getFolderInfo(state[0]);
//         	fullexpandedFolder = state[0];
//         	createFileTree();
